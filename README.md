...
"Simplified Technical English" standard), aimed at the verbose/hedging/vague
prose patterns common in AI-generated text. See
`specs/001-ste100-cli-checker/spec.md` for the full feature spec.

This repo has a second purpose: it's a working testbed for figuring out what
an effective **agentic development harness** needs — TDD enforcement, spec-
driven planning, context/token management, and session-quality checks, all
wired into one Claude Code project. If you're here for that, skip to
[Agentic harness notes](#agentic-harness-notes) and `notes.md`.

Every section below is written to be copy-pasted into a different project —
that's deliberate, per the tools' own design (each is config/skills dropped
into a repo, not a fork).
